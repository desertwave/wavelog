...
class User_Model extends CI_Model {
	...
	function add($username, $password, $email, $type, $firstname, $lastname, $callsign, $locator, $timezone,
		...
		$user_wwff_to_qso_tab, $user_pota_to_qso_tab, $user_sig_to_qso_tab, $user_dok_to_qso_tab, $user_station_to_qso_tab, $user_state_to_qso_tab,
		...
		if(!$this->exists($username)) {
			...
			$this->db->query("insert into user_options (user_id, option_type, option_name, option_key, option_value) values (" . $insert_id . ", 'qso_tab','state','show',".(xss_clean($user_state_to_qso_tab ?? 'off') == "on" ? 1 : 0).");");
	...
	function edit($fields) {
		...
		if(($this->session->userdata('user_type') == 99) || ($this->session->userdata('user_id') == $fields['id'])) {
			if($this->exists_by_id($fields['id'])) {
				...
				$this->db->query("replace into user_options (user_id, option_type, option_name, option_key, option_value) values (" . $fields['id'] . ", 'qso_tab','state','show',".(xss_clean($fields['user_state_to_qso_tab'] ?? 'off') == "on" ? 1 : 0).");");
